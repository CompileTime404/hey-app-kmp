...
import com.dorianbanic.chat.database.entities.ChatInfoEntity
...
import com.dorianbanic.chat.database.entities.ChatWithParticipants
...
interface ChatDao {
    ...
    @Upsert
    suspend fun upsertChats(chat: List<ChatEntity>)
    ...
    suspend fun upsertChatWithParticipantsAndCrossRefs(
        ...
        val crossRefs = participants.map {
            ChatParticipantCrossRef(chat.chatId, it.userId, true)
        }
        crossRefDao.upsertCrossRef(crossRefs)
    ...
    suspend fun upsertChatsWithParticipantsAndCrossRefs(
        ...
        val allCrossRefs = chats.flatMap { chatWithParticipants ->
            chatWithParticipants.participants.map { participant ->
                ChatParticipantCrossRef(chatWithParticipants.chat.chatId, participant.userId, true)
            }
        }
        crossRefDao.upsertCrossRef(allCrossRefs)

        chats.forEach { chat ->
            crossRefDao.syncChatParticipants(chat.chat.chatId, chat.participants)
        }
